...
names[names.index("Alex")] = "Alexander"
...
names_not_in_list = [name for name in names_to_look_for if name not in names]
...
total_runtime = 0
for runtime in film_runtimes.values():
...
all_titles = ''.join(film_runtimes_selected.keys())
unique_characters = set(all_titles)
...
# 4
first_three_letters = [name[:3] for name in names]
print(sorted(first_three_letters))
# 8
film_runtimes_selected = {
    "The Godfather": 175,
    "The Shawshank Redemption": 142,
...
film_titles = ["The Godfather", "The Shawshank Redemption", "The Dark Knight", "Inception", "Interstellar"]
film_runtimes = {title: film_runtimes_selected.get(title, None) for title in film_titles}
print(film_runtimes)
# 9
unique_characters = film_runtimes_selected = sorted(set(char for title in film_runtimes_selected.keys() for char in title if char != ' '))
